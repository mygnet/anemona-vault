<script lang="ts">
  ...
  let taskModalError = ''
  let deleteTaskPrompt: { index: number; title: string; code: string } | null = null
  ...
  $: filterCounts = {
    ...
    cancelled: localEntries.filter((entry) => entry.status === 'cancelled').length,
  }

  $: activeEntries = localEntries.filter((entry) => entry.status !== 'cancelled')
  $: totalProgress = activeEntries.length === 0
    ? 0
    : Math.round(activeEntries.reduce((sum, entry) => sum + Math.max(0, Math.min(100, Number(entry.progress) || 0)), 0) / activeEntries.length)
  ...
  function getRelativeDueLabel(entry: TodoEntry): string {
    ...
    const unit = units.find((item) => absoluteMinutes >= item.minutes) || units[units.length - 1]
    const value = Math.max(1, Math.round(absoluteMinutes / unit.minutes))
    const suffix = value === 1 ? unit.label : `${unit.label}s`

    return overdue ? `Overdue by ${value} ${suffix}` : `Due in ${value} ${suffix}`
  }
  ...
  function getProgressAccent(progress: number, status: TodoEntry['status']): string {
    if (status === 'cancelled') {
  ...
  function moveEntryToIndex(fromIndex: number, targetIndex: number) {
    if (fromIndex === targetIndex || fromIndex < 0 || targetIndex < 0 || fromIndex >= localEntries.length || targetIndex >= localEntries.length) {
  ...
  function toggleDone(index: number) {
    ...
    const progress = status === 'done' ? 100 : Math.min(current.progress || 0, 90)
  ...
  function confirmDeleteTask() {
    ...
    if (deleteTaskCodeInput.trim().toUpperCase() !== deleteTaskPrompt.code) return
  ...
  function saveTaskEdit() {
    ...
    if (taskModalMode === 'add') {
      localEntries = [
        ...localEntries,
        { title, progress: 10, status: 'open', priority: 'medium', dueAt: normalizeDueAt(editingTaskDueAt) },
      ]
    } else {
  ...
  function cyclePriority(index: number) {
    const current = localEntries[index]
    const priority = current.priority === 'low'
      ? 'medium'
...
<div class="todo-editor">
  <div class="editor-header">
    ...
    <button class="icon-btn primary-btn" on:click={openAddTaskModal} title="Add task"><span class="anemona icon-plus"></span></button>
  </div>
  ...
  <div class="filter-row">
    <button class="filter-chip" class:active={activeFilter === 'all'} on:click={() => (activeFilter = 'all')}>All {filterCounts.all}</button>
    <button class="filter-chip" class:active={activeFilter === 'open'} on:click={() => (activeFilter = 'open')}>Open {filterCounts.open}</button>
    <button class="filter-chip" class:active={activeFilter === 'done'} on:click={() => (activeFilter = 'done')}>Done {filterCounts.done}</button>
    <button class="filter-chip" class:active={activeFilter === 'cancelled'} on:click={() => (activeFilter = 'cancelled')}>Cancelled {filterCounts.cancelled}</button>
  </div>
  ...
  <div class="todo-list">
    {#if localEntries.length === 0}
      <div class="empty-state">Create your first task</div>
    {:else if visibleEntries.length === 0}
      <div class="empty-state">No tasks in this filter</div>
    {:else}
      {#each visibleEntries as item (item.entry)}
        {@const entry = item.entry}
        {@const index = item.index}
        <div class="todo-card" style={`--todo-progress-accent:${getProgressAccent(entry.progress, entry.status)};`} class:done={entry.status === 'done'} class:cancelled={entry.status === 'cancelled'} class:priority-high={entry.priority === 'high'} class:priority-medium={entry.priority === 'medium'} class:priority-low={entry.priority === 'low'} class:menu-open={activeMenuIndex === index} class:moving={movingTaskIndex === index} class:move-target={movingTaskIndex !== null && movingTaskIndex !== index} on:click={() => handleTaskCardClick(index)} on:keydown={(event) => handleTaskCardKeydown(index, event)} role="button" tabindex="0">
          <div class="todo-main-row">
            <button class="check-toggle" on:click={() => toggleDone(index)} title={entry.status === 'done' ? 'Mark as open' : 'Mark as done'}>
              <span class={`anemona ${entry.status === 'done' ? 'icon-checked' : 'icon-checkbox'}`}></span>
            </button>
            ...
            <div class="todo-copy">
              ...
              <div class="todo-meta">
                ...
                <button class={`priority-chip ${entry.priority}`} on:click={() => cyclePriority(index)} title="Change priority">{getPriorityLabel(entry)}</button>
                {#if entry.dueAt}
                  <span class={`todo-deadline ${getDueTone(entry)}`}>{getRelativeDueLabel(entry)}</span>
                {/if}
              </div>
            </div>
            ...
            <div class="todo-actions">
              <div class="menu-wrap">
                <button class="icon-btn state-btn" on:click|stopPropagation={() => toggleTaskMenu(index)} title="Task options">
                  <span class="anemona icon-dots-vertical"></span>
                </button>
                {#if activeMenuIndex === index}
                  <div class="menu-popover task-menu" use:smartPopover={{ open: activeMenuIndex === index, onClose: closeTaskMenu }}>
                    ...
                    <button class="menu-item" class:danger={entry.status !== 'cancelled'} on:click|stopPropagation={() => toggleCancelled(index)}>
                      <span class="anemona icon-x"></span>
                      <span>{entry.status === 'cancelled' ? 'Restore' : 'Cancel'}</span>
                    </button>
  ...
  {#if taskModalMode}
    <button class="task-modal-backdrop" on:click={cancelTaskEdit} aria-label="Close task edit"></button>
    <div class="task-modal">
      ...
      <div class="task-modal-actions">
        ...
        <button class="task-btn primary" on:click={saveTaskEdit}>{taskModalMode === 'add' ? 'Add' : 'Save'}</button>
      </div>
    </div>
  {/if}

  {#if deleteTaskPrompt}
    <button class="task-modal-backdrop" on:click={cancelDeleteTaskPrompt} aria-label="Close task delete confirmation"></button>
    <div class="task-modal">
      <h3>Delete task</h3>
      <p>Confirm deletion of <strong>{deleteTaskPrompt.title}</strong> by typing <strong>{deleteTaskPrompt.code}</strong></p>
      <input
      ...
      />
      <div class="task-modal-actions">
        <button class="task-btn" on:click={cancelDeleteTaskPrompt}>Cancel</button>
        <button class="task-btn danger" on:click={confirmDeleteTask}>Delete</button>
      </div>
    </div>
  {/if}
</div>
...
<style>
  ...
  .todo-card.cancelled .priority-chip {
    color: var(--vscode-sideBarTitle-foreground);
    border-color: color-mix(in srgb, var(--vscode-sideBarTitle-foreground) 10%, transparent);
  }
  ...
  .check-toggle {
    ...
    border: none;
    background: transparent;
    color: color-mix(in srgb, var(--todo-priority-color) 76%, var(--vscode-sideBarTitle-foreground));
    cursor: pointer;
    padding: 0;
    border-radius: 4px;
    flex-shrink: 0;
  ...
  .todo-card.cancelled .check-toggle {
    color: color-mix(in srgb, var(--vscode-sideBarTitle-foreground) 64%, transparent);
  }
  ...
  .priority-chip {
    ...
    color: color-mix(in srgb, var(--todo-priority-color) 64%, var(--vscode-sideBarTitle-foreground));
  ...
  .progress-slider::-webkit-slider-runnable-track {
    height: 0.18rem;
    border-radius: 999px;
    background: color-mix(in srgb, var(--vscode-sideBarTitle-foreground) 22%, transparent);
    border: 1px solid color-mix(in srgb, var(--todo-progress-accent) 14%, transparent);
  }
  ...
  .progress-slider::-webkit-slider-thumb {
    ...
    margin-top: -0.27rem;
    border-radius: 999px;
    border: 1px solid color-mix(in srgb, var(--todo-progress-accent) 24%, transparent);
